...
export async function POST(
  ...
  try {
    ...
    const limited = whatsAppRateLimitResponse(request, "delivery:enable", `${user.id}:${branchId}`);
    if (limited) return limited;
    return NextResponse.json(await WhatsAppAutomationService.enableDelivery({
      actorUserId: user.id,
      branchId,
    }));
  } catch (error) {
