...
export async function POST(
  ...
  try {
    ...
    const limited = whatsAppRateLimitResponse(request, "automation:disable", `${user.id}:${branchId}`);
    if (limited) return limited;
    return NextResponse.json(await WhatsAppAutomationService.disableAutomation({
      actorUserId: user.id,
      branchId,
    }));
  } catch (error) {
