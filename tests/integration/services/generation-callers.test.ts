...
import { createTestWorld, createStudent, createPayment } from "@/tests/factories";
...
describe("AI generation callers", () => {
  ...
  it("takes over an expired report owner at six minutes and rejects its late completion", async () => {
    ...
    const oldRun = runBranchAI(branch.id);
    ...
    await expect(runBranchAI(branch.id)).resolves.toMatchObject({ report: { marker: "current" } });
  ...
  it("preserves cooldown after failed publication in GET and POST metadata", async () => {
    ...
    await expect(draftOverdueMessages(branch.id)).rejects.toThrow("publication failed");
    const cached = await draftOverdueMessages(branch.id, { allowGeneration: false });
    const retry = await draftOverdueMessages(branch.id);
